...
class Permissions(db.Model):
    ...
    def __eq__(self, other):
        if isinstance(other, Permissions):
            return (self.create == other.create and
    ...
    def __eq__(self, other):
        if isinstance(other, User):
            return (
              ...
              self.fathers_name == other.fathers_name and
...
def get_client_by_id(client_id, client_type=None):
    if client_type:
       ...
       raise ClientNotFoundError(f"No {client_type} found with client_id = {client_id}")
...
def check_permission(client, entity, permission):
    ...
    if not getattr(permissions, permission):
        raise PermissionError(f"Client with id {client.id} does not have {entity}.{permission} permission")

    return True

def check_client_type(client_type):
  if client_type not in ['users', 'organisations']:
    raise WrongClientType(f'Client type {client_type} is wrong. Must be one of users, organisations')

def client_from_dict(client_dict, roles):
  client = {}
  client['role'] = list(
    filter(lambda r: r.name == client_dict['role'], roles)
  )[0]
  ...
  with open('data/roles.yaml', 'r') as f:
    for role_name, role_obj in yaml.safe_load(f).items():
      ...
      for entity, permissions_dict in role_obj.items():
        ...
        permission = list(
          filter(lambda p: p == permission, permissions)
        )[0]
...
def create_clients(client_type):
    try:
        ...
        if isinstance(e.orig, NotNullViolation):
          return jsonify({"status": "error", "message": "Not all client attributes are spcified"}), 400
